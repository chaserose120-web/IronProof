...
IronProofService is a technician job logger for heavy-duty equipment and automotive service work. Email/password accounts, user profiles, Solo/Crew job visibility, job-specific crews, job photos, and Heavy diagnostics files are stored in Supabase.

## Files

- `index.html` contains the app markup and form structure.
- `styles.css` contains the current visual design and responsive layout.
- `script.js` contains Supabase auth, profile loading, visibility-aware job loading/saving/editing/deleting, job-based crew collaboration, photo upload/delete, search/filter, detail rendering, and report behavior.
...
- `supabase/schema.sql` contains the Supabase profiles/jobs/job_crews/job_crew_members/job_photos/diagnostic_files tables, storage bucket setup, auth profile trigger, ownership triggers, migration from older account-wide crews, and Row Level Security policies.
...
Existing jobs default to Solo visibility. Crew jobs can be saved first, then the job detail screen can create a job-specific crew and share a join code. Crew leads and crew workers can edit the crew job; supervisors are view-only.
